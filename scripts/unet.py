...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if cond_channels is not None:
            self.cond_proj = nn.Sequential(
                nn.SiLU(),
                linear(cond_channels, time_embed_dim),
                nn.SiLU(),
                linear(time_embed_dim, time_embed_dim),
            )
        else:
    ...
    def forward(self, x, timesteps=None, context=None, y=None, cond_z=None):
        """
        ...
        if (cond_z is not None) and (self.cond_proj is not None):
            # cond_z: (B, Cc, h, w) -> cond_vec: (B, Cc)
            cond_vec = cond_z.mean(dim=(2, 3))
            emb = emb + self.cond_proj(cond_vec).type(emb.dtype)
