...
from model import VaeTestDataset as PredOccDataset
...
def preprocess_batch(batch, device=None):
    ...
    #input_occ_grid_map = input_gridMap.to_prob_map(TRESHOLD_P_OCC)
    # static map refined by local overlap across adjacent frames
    input_occ_grid_map = input_gridMap.to_prob_local_overlap_map(
        input_binary_maps,
        TRESHOLD_P_OCC,
        kernel_size=LOCAL_OVERLAP_KERNEL_SIZE
    )
...
def make_video(batch_out):
    """Convert preprocessed maps to a 3-channel sequence tensor: (B, T, 3, H, W).
    """
    ...
    dynamic = input_binary_maps * (1.0 - static_map)
    static = input_binary_maps * static_map
    free_past    = 1.0 - input_binary_maps
    data_3ch     = torch.cat([dynamic, static, free_past], dim=2)   # (B, T, 3, H, W)

    return data_3ch  # (B, T, 3, H, W)

def save_video_grid(video, output_path, sample_index=0):
    """Save one sample from a (B, T, 3, H, W) tensor as a single image grid."""
    os.makedirs(os.path.dirname(output_path), exist_ok=True)

    frames = video[sample_index]
    fig = plt.figure(figsize=(8, 1))
    fontsize = 8

    for frame_index in range(frames.size(0)):
        axis = fig.add_subplot(1, frames.size(0), frame_index + 1)
        image = make_grid(frames[frame_index].detach().cpu()).permute(1, 2, 0)
        axis.imshow(image)
        axis.set_xticks([])
        axis.set_yticks([])
        axis.set_title(f"n={frame_index + 1}", fontdict={"fontsize": fontsize})
...
def main(argv):
    if len(argv) < 1 or len(argv) > 3:
        print("usage: python rgbdata_preprocessing.py [DATA_DIR] [SPLIT=test] [OUTPUT_DIR=./output/rgb_preprocessing]")
        return False

    data_dir = argv[0]
    split = argv[1] if len(argv) >= 2 else "test"
    output_dir = argv[2] if len(argv) >= 3 else "./output/rgb_preprocessing"

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    dataset = PredOccDataset(data_dir, split)
    ...
    for batch_index, batch in enumerate(dataloader):
        ...
        video = make_video(batch_out)
        output_path = os.path.join(output_dir, f"video_{batch_index:05d}.jpg")
        save_video_grid(video, output_path)
